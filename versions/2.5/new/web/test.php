...
$row = array('siteBuilderId'=>$id, 'name'=>'Some other test. ' . microtime());
...
// Use the dynamic table name finder
$dm->SiteUser()->add(array('siteUserId'=>1, 'username'=>'abc', 'password'=>'abc'));
$dm->SiteUser()->add(array('siteUserId'=>2, 'username'=>'def', 'password'=>'def'));
$dm->SiteUserInfo()->add(array('siteUserId'=>1, 'info'=>'This is user 1'));
$dm->SiteUserInfo()->add(array('siteUserId'=>2, 'info'=>'This is user 2'));
$joins = array(array('tableName'=>'##SiteUserInfo##', 'alias'=>'ui', 'use'=>array('a.siteUserId', 'ui.siteUserId')));
$results = $dm->SiteUser()->findBy(array('joins'=>$joins));
$tests[] = array('test'=>'Use dynamic table name finder', 'results'=>'<pre>'.print_r($results, true).'</pre>');
...
// Lookup a row using the dynamic find using a field with upper case letters
$results = $dm->SiteUserInfo()->findOneBySiteUserId(1);
$tests[] = array('test'=>'Use dynamic find for one row', 'results'=>'<pre>'.print_r($results, true).'</pre>');
