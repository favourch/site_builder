...
class base_db extends util
{
	...
	const DEBUG_ERROR = 1;
	const DEBUG_ALL = 2;
	const DEBUG_DETAILED = 3;

	function base_db($host, $user, $password, $name, $debug=null)
	{
	...
	function query($query)
	{
		...
		// Debug and check for a MySQL error.
		if($this->debug == self::DEBUG_ALL) {
			echo "<div>$query</div>";
		}
		if($this->debug == self::DEBUG_DETAILED) {
			$this->debug();
		}
		if(mysql_error()) {
			switch($this->debug) {
	...
	function debug($return=false)
	{
		$html = <<<EOT
<blockquote>
<font face=arial size=2 color=000099><b>Query --</b> [<font color=000000><b>$this->lastQuery</b></font>]</font>
...
EOT;
	...
	<table cellpadding=5 cellspacing=1 bgcolor=555555>
		<tr bgcolor=eeeeee>
			<td nowrap valign=bottom><font color=555599 face=arial size=2><b>(row)</b></font></td>
EOT;

			for ( $i=0; $i < count($this->colInfo); $i++ ) {
				$html .= "\t\t\t<td nowrap align=left valign=top><font size=1 color=555599 face=arial>{$this->colInfo[$i]->type} {$this->colInfo[$i]->max_length}</font><br><font size=2><b>{$this->colInfo[$i]->name}</b></font></td>\n";
			}
			...
			if($this->lastResult) {
				...
				foreach($this->get_results(null,'ARRAY_N') as $one_row) {
					$i++;
					$html .= "\t\t<tr bgcolor=ffffff>\n\t\t\t<td bgcolor=eeeeee nowrap align=middle><font size=2 color=555599 face=arial>$i</font></td>\n";
					foreach ( $one_row as $item ) {
						$html .= "\t\t\t<td nowrap><font face=arial size=2>$item</font></td>\n";
					}
					$html .= "\t\t</tr>\n";
				}
				...
				$html .= "\t\t<tr bgcolor=ffffff>\n\t\t\t<td colspan=".(count($this->colInfo)+1)."><font face=arial size=2>No Results</font></td>\n\t\t</tr>\n";
			}
			...
			$html .= "\t<font face=arial size=2>No Results</font>\n";
		}
