...
class base_data_util extends util
{
	...
	private function getSqlJoins($sql, $joins=array())
	{
		if($joins) {
			foreach($joins as $j) {
	...
	private function getSqlWhere($sql, $fields=array())
	{
		$where = array();
	...
	private function getSqlGroup($sql, $group=array())
	{
		if(count($group) > 0) {
	...
	private function getSqlOrder($sql, $order=array())
	{
		if(count($order) > 0) {
	...
	private function getSqlLimit($sql, $limit=0)
	{
		if($limit) {
			$sql .= " limit $limit";
		}
